...
def compute_masked_vegetationindex(
    ...
    if soil_detection:
        ...
        tile.add_dirpath("SoilAnomaliesDir", tile.data_directory / "SoilAnomalies") #Choose anomalies directory # added 
...
# edited
        ...
        tile.used_bands, tile.vi_formula = get_bands_and_formula(vi, path_dict_vi = path_dict_vi, forced_bands = ["B2","B3","B4", "B8", "B8A","B11", "B12"] if soil_detection else get_bands_and_formula(formula = formula_mask)[0]) #Selects only relevant bands depending on used vegetation index plus forced_bands used in masks
        ...
        for date_index, date in enumerate(tile.dates):
            if date in new_dates:
                ...
                # this was added
                vegetation_index = vegetation_index.where(vegetation_index > -1,0)
                vegetation_index = vegetation_index.where(vegetation_index < 1,0)
                ...
                if soil_detection:
                    ...
                    source_mask = get_source_mask(tile.paths["Sentinel"][date], sentinel_source, extent = tile.raster_meta["extent"]) #Masking with source mask if option chosen
                    mask, soil = compute_masks(stack_bands, soil_data, date_index, source_mask)
                    write_tif(soil, tile.raster_meta["attrs"], tile.paths["SoilAnomaliesDir"] / str("SoilAnomaly_" + date + ".tif"),nodata=0)
                else:
