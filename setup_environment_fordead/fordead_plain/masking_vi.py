...
# edited
    ...
    # original # soil_anomaly = compute_vegetation_index(stack_bands, formula = "(B11 > 1250) & (B2 < 600) & ((B3 + B4) > 800)")
    #soil_anomaly = compute_vegetation_index(stack_bands, formula = "(B11 > 2000) & (B2 < 900) & ((B3 + B4) > 1500) & ((B2 + B3 + B4 + B11) > 3500)")
    #soil_anomaly = compute_vegetation_index(stack_bands, formula = "((B4>800) & (B2<1000) & (B3<B4))") # | (((B4 > 400) & (B4 < 800) & (B11 > 1500) & (B11 < 3300)) & (((B8-B4)/(B8+B4)) > 0.5))")
    
    soil_anomaly = compute_vegetation_index(stack_bands, formula = "((((B2-B12)/(B2+B12))) < -0.25) & (((B4+B2-B3)/(B4+B3+B2)) > 0.23) & (((B8-B4)/(B8+B4)) < 0.65) & (3000 < (0.3037 * B2 + 0.2793 * B3 + 0.4743 * B4 + 0.5585 * B8 + 0.5082 * B11 + 0.1863 * B12))")
    
    # soil_anomaly = compute_vegetation_index(stack_bands, formula = "(B11 > 1250) & (B2 < 600) & (B4 > 600)")
    # soil_anomaly = compute_vegetation_index(stack_bands, formula = "(B4 + B2 - B3)/(B4 + B2 + B3)") #Bare soil index

    shadows = (stack_bands==0).any(dim = "band") # should edit to any < 0
    outside_swath = stack_bands.isel(band=0)<0 # does this still make sense?
    inva = compute_vegetation_index(stack_bands, formula = "(((B8-B4)/(B8+B4)) > 1) | (((B8-B4)/(B8+B4)) < -1) | (B3 <= 0) | (B4 <=0)") #| (2500<(-0.2848 * B2 - 0.2435 * B3 - 0.5436 * B4 + 0.7243 * B8 + 0.0840 * B11 - 0.1800 * B12))")

    
    #original b2 threshold is 600
    #750 was working ok
    #removed highly visible clouds, duplicate of cloud detection - integrated partially in soil anomaly wiht b2<1000
    invalid = shadows | outside_swath | inva
#   invalid = shadows | outside_swath | (stack_bands.sel(band = "B2") >= 600)
...
# edited
    ...
    #increase counter
    soil_data["count"]=xr.where(~invalid & soil_anomaly,soil_data["count"]+1,soil_data["count"])
    #originally: reset counter if no anomaly
    #soil_data["count"]=xr.where(~invalid & ~soil_anomaly,0,soil_data["count"])
    #edited to reset count only for pixels which were not classified as soil in "state"
    soil_data["count"]=xr.where(~invalid & ~soil_anomaly & ~soil_data["state"],0,soil_data["count"])
# edited count soil from 3 to current number - only affects the pixels where the soil is reached
    #soil_data["switch"] = xr.where(soil_data["count"] == 3 & ~soil_data["state"], True, False)

    soil_data["state"] = xr.where(soil_data["count"] == 4, True, soil_data["state"])
...
def compute_masks(stack_bands, soil_data, date_index, source_mask):
    """
    ...
    #Combine all masks
    mask = shadows | clouds | outside_swath | soil_data['state'] | invalid # |soil_anomaly # commented and added invalid

    return mask, ~invalid & soil_anomaly# added return soil anomaly
